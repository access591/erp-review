 ...
 * org.springframework.util.FileSystemUtils; import
 * org.springframework.web.multipart.MultipartFile;
 * 
 * import com.access.erp.service.FilesStorageService;
 * 
 * 
 * @Service public class FilesStorageServiceImpl implements FilesStorageService
 * {
 * 
 * private final Path root = Paths.get("static/upload");
 * 
 * @Override public void init() { try { Files.createDirectory(root); } catch
 * (IOException e) { throw new
 * RuntimeException("Could not initialize folder for upload!"); } }
 * 
 * @Override public void save(MultipartFile file) { try {
 * Files.copy(file.getInputStream(),
 * this.root.resolve(file.getOriginalFilename())); } catch (Exception e) {
 * System.out.println("error : "+ e.getMessage()); //throw new
 * RuntimeException("Could not store the file. Error: " + e.getMessage()); } }
 * 
 * @Override public Resource load(String filename) { try { Path file =
 * root.resolve(filename); Resource resource = new UrlResource(file.toUri());
 * 
 * if (resource.exists() || resource.isReadable()) { return resource; } else {
